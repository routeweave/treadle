...
| `build-config` | `usr/libexec/treadle/` | Translates UCI plus subscription node files into a sing-box JSON config. The only writer of that config. |
...
working config and logs why. The running config lives on tmpfs, so a
